
## Network

**Definition:**
A network in computer terms is a group of devices connected together so they can communicate with each other. This connection is established using various technologies like cables or wireless signals.

**Example:**
Imagine you have a home network where your computer, printer, and smartphone are all connected. This forms a small network.

---

#### Netmask (Subnet Mask)

**Definition:**
A netmask, also known as a subnet mask, is used to divide an IP address into network and host portions. It helps in determining which part of an IP address represents the network and which part represents individual devices (hosts) on that network.

**Example:**

* **IP Address:** 192.168.1.10
* **Subnet Mask:** 255.255.255.0

In this example, the subnet mask `255.255.255.0` means the first 24 bits of the IP address are the network part, and the remaining 8 bits are for hosts.

---

#### Subnetting

**Definition:**
Subnetting is the process of dividing a larger network into smaller, more manageable sub-networks (subnets). It helps in organizing and optimizing the use of IP addresses.

**How It Works:**

* **Network Address:** Identifies the overall network.
* **Subnet Mask:** Defines the boundary between the network part and the host part of an IP address.
* **Subnet Address:** Identifies individual sub-networks within the larger network.

---

**Why Use a Subnet Mask?**

Managing a large network with numerous hosts can be challenging due to:

1. **Maintenance Complexity:** Handling a vast number of devices becomes cumbersome.
2. **Security Concerns:** Without segmentation, it's harder to isolate departments or groups within an organization.

**Solution: Subnetting**

By dividing a large network into smaller subnets:

* **Improved Management:** Each subnet can be managed independently.
* **Enhanced Security:** Departments or groups can be isolated, reducing potential security breaches.
* **Efficient IP Usage:** IP addresses are allocated more effectively, minimizing waste.

---

**Subnet Mask and CIDR (Classless Inter-Domain Routing)  Notation Examples:**

| CIDR Notation | Subnet Mask     | Usable Hosts | Description                          |
| ------------- | --------------- | ------------ | ------------------------------------ |
| /8            | 255.0.0.0       | 16,777,214   | Very large networks (Class A)        |
| /16           | 255.255.0.0     | 65,534       | Medium-sized networks (Class B)      |
| /24           | 255.255.255.0   | 254          | Small networks (Class C)             |
| /26           | 255.255.255.192 | 62           | Divides a /24 network into 4 subnets |
| /30           | 255.255.255.252 | 2            | Point-to-point links (routers)       |

---

**Subnetting Example:**

Suppose you have a network with the IP address `192.168.1.0` and a subnet mask of `255.255.255.0`. This gives you one subnet with 254 usable IP addresses.

If you subnet this into smaller networks, you could have:

* **Subnet 1:** `192.168.1.0/25`
  IP Range: `192.168.1.1` to `192.168.1.126`
  (Usable Hosts: 126)

* **Subnet 2:** `192.168.1.128/25`
  IP Range: `192.168.1.129` to `192.168.1.254`
  (Usable Hosts: 126)

This way, you use your IPs more efficiently by creating smaller groups.

---

**Representation of Subnet Masks:**

A subnet mask is a 32-bit number made up of 1s and 0s:

* **1s:** Represent the network portion.
* **0s:** Represent the host portion.

**Example:**

```
Subnet Mask (Binary): 11111111.11111111.11111111.11000000
Subnet Mask (Decimal): 255.255.255.192
```

This indicates that the first 26 bits are for the network, and the remaining 6 bits are for hosts.

---

**How Subnetting Works:**

Without subnetting, identifying a device requires:

1. Identification of the network.
2. Identification of the host.
3. Identification of the process.

With subnetting, the process becomes:

1. Identification of the network.
2. Identification of the subnet.
3. Identification of the host.
4. Identification of the process.

This additional layer (subnet) allows for better organization and management of IP addresses.

---

**Benefits of Subnetting:**

* **Efficient IP Address Management:** Reduces wastage of IP addresses.
* **Improved Network Performance:** Limits broadcast domains, reducing traffic.
* **Enhanced Security:** Isolates different network segments.
* **Simplified Troubleshooting:** Easier to pinpoint issues within specific subnets.

---

#### Reserved IP Addresses

A reserved IP address is an IP address that is assigned to your account and remains available for you unless you release it. You can use reserved IP addresses for various purposes.

**Reserved IP Address**
A reserved IP address is an IP address that has been assigned to your account and is available for your use until you release it. You can use reserved IP addresses for different purposes.

---

